...
function verifyToken(token) {
  try {
    ...
    if (!/^\d+$/.test(ts)) return false;                 // ts doit être purement numérique
    if (!/^[0-9a-f]{64}$/.test(sig)) return false;        // sig = HMAC-SHA256 en hex
    if (Date.now() - parseInt(ts, 10) > SESSION_TTL) return false;
...
// ---- Limiteur de débit (table Supabase `rate_limits`) -------------------
// Renvoie true si la requête est autorisée, false si la limite est atteinte.
// IMPORTANT : en cas d'erreur (ex. table absente), on FAIL-OPEN — on n'enferme
// jamais l'utilisateur dehors à cause du limiteur. La table est recommandée
// (voir SETUP.md) pour que la limitation soit réellement active.
async function rateLimit(key, max, windowMs) {
  try {
    const now = Date.now();
    const k = encodeURIComponent(key);
    const rows = await sb(`rate_limits?key=eq.${k}&select=key,count,window_start`);
    const row = rows && rows[0];
    if (!row) {
      await sb('rate_limits', {
        method: 'POST',
        headers: { Prefer: 'resolution=merge-duplicates' },
        body: { key, count: 1, window_start: new Date(now).toISOString() },
      });
      return true;
    }
    const start = new Date(row.window_start).getTime();
    if (now - start > windowMs) {
      await sb(`rate_limits?key=eq.${k}`, { method: 'PATCH', body: { count: 1, window_start: new Date(now).toISOString() } });
      return true;
    }
    if (row.count >= max) return false;
    await sb(`rate_limits?key=eq.${k}`, { method: 'PATCH', body: { count: row.count + 1 } });
    return true;
  } catch (e) {
